/// Zig "compile_commands.json" file generator.
/// Initial implementation done by `the-argus` at
...
pub fn generateFromInstallStep(b: *std.Build) void {
    ...
    var cs: std.ArrayList(*Step.Compile) = .empty;

    for (install_step.dependencies.items) |step| {
        switch (step.id) {
            .install_artifact => {
                const ia = step.cast(Step.InstallArtifact).?;
                cs.append(allocator, ia.artifact) catch @panic("OOM");
            },
...
fn getCSources(b: *std.Build, steps: []const *std.Build.Step.Compile) []*AbsoluteCSourceFiles {
    ...
    var res = std.ArrayList(*AbsoluteCSourceFiles).initCapacity(allocator, 0) catch @panic("OOM");
    ...
    while (index < compile_steps_list.items.len) {
        ...
        for (extractIncludeDirsFromCompileStep(b, step)) |include_dir| {
            shared_flags.append(b.allocator, includeFlag(b.allocator, include_dir)) catch @panic("OOM");
        }
        ...
        for (step.root_module.link_objects.items) |link_object| {
            switch (link_object) {
                ...
                .c_source_files => {
                    ...
                    var flags: std.ArrayList([]const u8) = .empty;
                    flags.appendSlice(b.allocator, source_files.flags) catch @panic("OOM");
                    flags.appendSlice(b.allocator, shared_flags.items) catch @panic("OOM");
                    source_files.flags = flags.toOwnedSlice(b.allocator) catch @panic("OOM");
...
fn makeCdb(step: *std.Build.Step, _: std.Build.Step.MakeOptions) anyerror!void {
    ...
    var compile_commands = std.ArrayList(CompileCommandEntry).initCapacity(allocator, 0) catch @panic("OOM");
